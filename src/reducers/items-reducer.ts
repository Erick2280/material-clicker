...
const itemsReducer = createReducer(
  {} as RItems,
  {
    ...
    [GIVE_ITEM.type]: (state, { payload }: ReturnType<typeof GIVE_ITEM>) => {
      state[payload.item.id] = (state[payload.item.id] || 0) + payload.count;
    },
    ...
    [TAKE_ITEM.type]: (state, { payload }: ReturnType<typeof TAKE_ITEM>) => {
      state[payload.item.id] = Math.max(0, (state[payload.item.id] || 0) - payload.count);
    },
    ...
    [SET_ITEM.type]: (state, { payload }: ReturnType<typeof SET_ITEM>) => {
      state[payload.item.id] = Math.max(0, payload.count);
    }
  }
);
